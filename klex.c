...
{
	if (state->text == NULL) {
		state->text = malloc(1024);
		memset(state->text, 0, 1024);
		state->capacity = 1024;
	}
	if (state->size >= state->capacity - 1) {
		state->capacity *= 2;
		state->text = realloc(state->text, state->capacity);
...
static char* intern_text(KlexState* state)
{
	...
	if (state->interned == NULL) {
		...
		state->interned = malloc(1024 * sizeof(char*));
	...
	if (state->num_interned >= state->cap_interned) {
		...
		state->interned = realloc(state->interned, state->cap_interned * sizeof(char*));
	}
